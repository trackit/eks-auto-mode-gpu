...
ENV CMDARGS --listen 0.0.0.0 --port 8080
...
RUN pip install --no-cache-dir --timeout=600 -r /tmp/requirements_docker.txt -r /tmp/requirements_versions.txt && \
...
CMD [ "sh", "-c", "/content/entrypoint.sh --listen" ]
